...
import { parseMediaContent, isEmojiOnly } from "../media.js";
...
export default function ChatWindow() {
  ...
  // "Coming soon" toast for the call buttons: auto-dismisses, also dismissible.
  const [soonCall, setSoonCall] = useState(null); // "voice" | "video"
  const soonTimer = useRef(null);
  useEffect(() => {
    if (!soonCall) return;
    soonTimer.current = setTimeout(() => setSoonCall(null), 2500);
    return () => clearTimeout(soonTimer.current);
  }, [soonCall]);
  ...
  return (
    <main className="flex min-w-0 flex-1 flex-col h-full relative" style={{ background: "var(--nm-bg)" }}>
      {/* Header */}
      <header className="nm-raised-sm relative flex items-center gap-3 rounded-b-2xl px-4 py-3 shrink-0 z-10">
        <button
          onClick={() => useStore.getState().openChat(null)}
          className="md:hidden nm-icon-btn h-9 w-9"
        >
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => setSoonCall("voice")}
            className="nm-icon-btn nm-icon-btn-soon h-9 w-9"
        ...
        {/* "Coming soon" toast, anchored near the call buttons */}
        {soonCall && (
          <div
            onClick={() => setSoonCall(null)}
            className="nm-raised-sm absolute right-3 top-full z-20 mt-2 cursor-pointer rounded-xl px-3 py-2 text-xs font-medium"
            style={{ color: "var(--nm-text)" }}
            role="status"
          >
            {soonCall === "voice" ? "Voice calls coming soon" : "Video calls coming soon"}
          </div>
        )}
      </header>
...
function MessageBubble({ message }) {
  ...
  const emojiOnly = !media && isEmojiOnly(message.content);
  ...
  return (
    <div className={`flex flex-col ${own ? "items-end" : "items-start"} px-1`}>
      <div
        className={`${
          emojiOnly
            ? "relative max-w-[70%] rounded-2xl px-2 py-1 text-4xl leading-none"
            : "max-w-[70%] rounded-2xl px-4 py-2.5 text-[15px]"
        } ${
          own
            ? isFailed
              ? "rounded-br-sm"
      ...
      >
        {media ? (
          ...
          <p className={`whitespace-pre-wrap break-words ${emojiOnly ? "select-none" : ""}`}>
            {message.content}
          </p>
        )}
        <div className={`mt-1 flex items-center justify-end gap-1 text-[11px] ${emojiOnly ? "absolute -bottom-1 -right-1 rounded-full bg-black/30 px-1.5 text-white" : ""}`} style={emojiOnly ? undefined : { color: "var(--nm-text-faint)" }}>
